...
//
#ifndef MYBOOST_LOCALE_UTF_HPP_INCLUDED
#define MYBOOST_LOCALE_UTF_HPP_INCLUDED
...
#ifndef MYBOOST_NAMESPACE
#define MYBOOST_NAMESPACE boost
#endif

namespace MYBOOST_NAMESPACE {
namespace locale {
///
...
namespace utf {
    ...
    struct utf_traits<CharType,4> {
        ...
        {
            if(BOOST_LOCALE_UNLIKELY(current == last))
                return MYBOOST_NAMESPACE::locale::utf::incomplete;
            code_point c=*current++;
            if(BOOST_LOCALE_UNLIKELY(!is_valid_codepoint(c)))
                return MYBOOST_NAMESPACE::locale::utf::illegal;
            return c;
        }
